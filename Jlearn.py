...
#grid_txt(B,l)
    ...
    def hebbstrenght(self): #generates an hebbian CM
        ...
        self.hebbJ=self.hebbJ/np.norm(self.hebbJ)
    ...
    def J_txt(A,l,eta,epochs): #prints J in a .txt reporting the trajectory,learning rate and epochs of the learning
        ...
        file=np.open(name_of_file + ".txt","w+")
        for line in B:
            np.savetxt(file,line,fmt='%.2f')
        file.close()
    
    def grid_txt(B,l): #prints grid in a .txt reporting the l parameter for randomness
        A=np.matrix(B)
        name_of_file="grid("+np.str(np.len(B))+","+np.str(l)+")"
        file=np.open(name_of_file + ".txt","w+")
        for line in A:
            np.savetxt(file,line,fmt='%.2f')
        file.close()            
