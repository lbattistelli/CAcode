...
#sample_regular_trajectory(self,L,n,epochs)
    ...
    def kernel(self,r_i,r_r):#r_i=position on the grid,r_r=rat position,sig=standard deviation,c=cutoff
        ...
        if d<=self.c:
            V=exp(-0.5*pow(d/self.sig,2))
        else:
            V=0    
        return V

    def computeV(mapping,currentposition): #computes the activity of the network depending on the current position of the external input
        N=np.len(mapping)    
        V=np.zeros(N)
        for i in range(N):
            V[i]=kernel(mapping[i],currentposition,self.sig,self.c)
        return V
    ...
    def overlap(matA,matB):
        ...
        m=m/float(np.norm(matA)*np.norm(matB))
        return m
...
#interaction kernel
    ...
    def fix_parameters(V1,h1,th,a,a2):
        ...
        while (not fixed) and it<maxiter:
            ...
            V1=np.asarray(np.list(np.map(lambda h1: transfer(h1,th,g),h1)))
...
#initialization
        ...
        for step in range(maxsteps):
            h=np.dot(J,V)
            V=np.asarray(np.list(np.map(lambda h: transfer(h,th,g),h)))
